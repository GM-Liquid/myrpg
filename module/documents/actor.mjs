   ...
   */
    ...
    for (let [x, skill] of Object.entries(systemData.skills)) {
      ...
      skill.mod = skill.c;
        ...
        // ƒл€ каждого навыка убеждаемс€, что его значение не превышает значение св€занной способности
        for (let [key, skill] of Object.entries(systemData.skills)) {
            ...
            if (systemData.abilities[abilityKey]) {
                ...
                // ≈сли значение навыка превышает значение способности, устанавливаем его равным значению способности
        ...
        // –ассчитываем максимальные очки здоровь€ по формуле: 10 + 5 * (телосложение + сила)
        systemData.health.max = 10 + 5 * (systemData.abilities.con.value + systemData.abilities.will.value);

        // ≈сли текущие ќ« отсутствуют или превышают максимум, устанавливаем их равными максимуму
        if (!systemData.health.value || systemData.health.value > systemData.health.max) {
            systemData.health.value = systemData.health.max;
        }

        // –ассчитываем значение ранени€ как половину от макс. ќ«, округлЄнную вниз
