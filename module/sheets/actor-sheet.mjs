                    ...
                    html.find("textarea.rich-editor").each(function () {
                        if (!this._tinyMCEInitialized) {
                            tinymce.init({
                                ...
                                branding: false,
                                statusbar: false,
                                // Добавляем autoresize для динамического изменения высоты
                                plugins: "autoresize link lists contextmenu",
                                toolbar: false,
                                contextmenu: "bold italic underline strikethrough link bullist numlist",
                                // Наследуем стили, чтобы редактор выглядел как стандартное поле
                                content_style: "body { margin: 0; padding: 0; font-family: inherit; font-size: inherit; color: #1b1210; }",
                                // Задаём минимальную высоту (начальное значение), при этом при наборе текста высота будет расти
                                autoresize_min_height: 40,
                                // Обеспечиваем, что редактор будет использовать всю доступную ширину контейнера
                                width: "100%",
                                setup: function (editor) {
                                    editor.on("init", function () {
                                        // Дополнительную настройку можно добавить здесь, если потребуется
                                    });
                                }
                            });
